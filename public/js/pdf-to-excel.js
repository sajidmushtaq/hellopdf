document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderFile() {
    ...
    card.className = "pdf-excel-file-card";
  ...
  convertBtn?.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch("/pdf-to-excel", {
        method: "POST",
        body: formData
      });
      ...
      if (convertedExcelUrl) URL.revokeObjectURL(convertedExcelUrl);
      convertedExcelUrl = URL.createObjectURL(blob);
  ...
  downloadBtn?.addEventListener("click", () => {
    if (!convertedExcelUrl) {
      alert("Excel file is not ready yet");
      return;
    }

    const a = document.createElement("a");
    a.href = convertedExcelUrl;
