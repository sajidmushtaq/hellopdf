document.addEventListener("DOMContentLoaded", () => {
  ...
  function addFile(files) {
    const file = Array.from(files || []).find(isWordFile);
  ...
  function renderFile() {
    ...
    card.className = "word-pdf-file-card";

    card.innerHTML = `
      <button class="remove-file-btn" type="button">×</button>

      <div class="word-file-preview">
...
formData.append("user_id", data.user.id);
  ...
  alert(errorText || "Word to PDF conversion failed");
  ...
  return;
      ...
      if (!blob || blob.size < 100 || blob.type !== "application/pdf") {
        alert("Conversion failed. Please try another Word file.");
        return;
      }
